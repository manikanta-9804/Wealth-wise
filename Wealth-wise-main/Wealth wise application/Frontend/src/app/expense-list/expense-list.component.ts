...
export class ExpenseListComponent implements OnInit{
  ...
  constructor(private dialog: MatDialog, private addExpenseService: AddExpenseService, private fb: FormBuilder, private shared: SharedService, private _router: Router){}
  ...
  calculateTotal(category: string): number {
    ...
    this.totalAmount = this.expenseData.filter(item => item.category === category).reduce((sum, item) => sum + item.amount, 0);
  ...
  reloadComponent(){
    this._router.navigateByUrl('/expense-list', {skipLocationChange: true}).then(() => {
