...
routing.post('/api/signup', async (req, res, next) => {
    try{
        ...
        res.status(200).json({user, message : "User signed up successfully: " + userObj.userId})
    }catch (err){
        next(err);
    }
})
...
routing.post('/api/login', async (req, res, next) => {
    try{
        ...
        if(user){
            return res.status(200).json({user, message: "User logged in successfull"});
        }else{
...
routing.post('/addExpenses', async (req, res, next) => {
    try{
        ...
        if(expense){
            return res.status(200).json({expense, message:`Expense added successfully: ${expenseObj.expenseId}`});
        }else{
...
routing.put('/updateExpense/:expenseId', async (req, res, next) => {
    try{
        ...
        return res.status(200).json({result, message:"expense updated successfully: " + expenseId})
    }catch(err){
        next(err)
    }
})
